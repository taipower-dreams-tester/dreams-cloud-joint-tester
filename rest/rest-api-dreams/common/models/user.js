...
const DEFAULT_TTL = 3 * 31536000; // sync with client keep me sign in period
...
const { getOwnerKey } = require('../../server/constant');
const { setDefaultMenuPermission } = require('../utils/permissionUtil');
 ...
 * See [Partitioning users with realms](http://loopback.io/doc/en/lb2/Partitioning-users-with-realms.html) for details.
...
module.exports = function(User) {
  ...
  *    });
  ...
  User.prototype.verify = function(options, fn) {
    ...
    const tokenGenerator = options.generateVerificationToken || User.generateVerificationToken;
  ...
  function createVerificationEmailBody(options, cb) {
    const template = loopback.template(options.template);
  ...
  User._invalidateAccessTokensOfUsers = function(userIds, options, cb) {
    ...
    const isRelationPolymorphic = relatedUser.polymorphic && !relatedUser.modelTo;
  ...
  User.setup = function() {
    ...
    UserModel.beforeRemote('create', function(ctx, user, next) {
      ...
      // create portalUser
      const app = UserModel.app;
      app.models.PortalUser.create({ email: body.email, ownerKey: getOwnerKey() }, (err) => {
        next(err);
      });
    });

    UserModel.beforeRemote('deleteById', function(ctx, user, next) {
      const userId = ctx.req.params.id;
      UserModel.findById(userId, (err, user) => {
        const app = UserModel.app;
        app.models.PortalUser.destroyAll({ email: user.email }, (err) => {
          next(err);
        });
      });
    });

    UserModel.beforeRemote('prototype.patchAttributes', function(ctx, user, next) {
      const userId = ctx.req.params.id;
      const newUser = ctx.req.body;
      UserModel.findById(userId, (err, user) => {
        const app = UserModel.app;
        if (newUser.role && user.role !== newUser.role) {
          (async () => await setDefaultMenuPermission(app.models, userId, newUser.role))();
        }
        if (newUser.email && newUser.email !== '' && user.email !== newUser.email) {
          err = new Error("Email is read-only");
          err.statusCode = 403;
          next(err);
        } else {
          next(err);
    ...
    UserModel.afterRemote('prototype.patchAttributes', function(ctx, inst, next) {
      const models = UserModel.app.models;
      const userRole = inst.role;
      const userId = inst.id;
      Promise.all([
        models.Role.find({where: {name: userRole}}),
        models.DataPermission.findOne({ where: { and: [{ dataKey: 'plantNo' }, { userId }] } }),
      ])
        .then(([roles, permission]) => {
          if (roles.length === 0) {
            throw { error: 'no roles existed' };
          }

          const where = { and: [{ principalId: inst.id }, { principalType: 'USER' }] };
          return Promise.all([
            models.RoleMapping.updateAll(where, { roleId: roles[0].id }),
            _.includes(['admin', 'powerUser'], userRole) ?
              permission.updateAttributes({ allowedValues: '{"values":"ALL"}' }) :
              Promise.resolve(`no need to update permission in this role: ${userRole}`),
          ]);
        })
        .then(() => {
          next();
    ...
    UserModel.afterRemote('create', function(ctx, inst, next) {
      const app = UserModel.app;
      app.models.Role.find({where: {name: inst.role}}, (err1, role) => {
        ...
        Promise.all([
          app.models.RoleMapping.create({ principalType: 'USER', principalId: inst.id, roleId: role[0].id }),
          app.models.DataPermission.create({ userId: inst.id, dataKey: "plantNo", allowedValues: '{"values":"ALL"}'}),
          setDefaultMenuPermission(app.models, inst.id, role[0].name),
          UserModel.onCreate ? UserModel.onCreate(inst.id, inst.role) : Promise.resolve(),
        ]).then(() => {
          next();
        });
      });
    ...
    UserModel.afterRemote('deleteById', function(ctx, inst, next) {
      const userId = ctx.req.params.id;
      const { RoleMapping, DataPermission, MenuPermission } = UserModel.app.models;
      Promise.all([
        RoleMapping.destroyAll({ and: [{ principalId: userId }, { principalType: 'USER' }] }),
        DataPermission.destroyAll({ userId }),
        MenuPermission.destroyAll({ userId }),
        UserModel.onDelete ? UserModel.onDelete(userId) : Promise.resolve(),
      ]).then(() => {
        next();
      }).catch(next);
    });

    UserModel.afterRemote('find', function(ctx, inst, next) {
      const loopBackCtx = LoopBackContext.getCurrentContext();
      const accessToken = loopBackCtx && loopBackCtx.get('accessToken');
      if (!accessToken) {
        ctx.result = Array.isArray(ctx.result) ? [] : null;
        next();
        return;
      }
      _.remove(ctx.result, user => user.username === 'machineadmin');
      next();
    });
  ...
  User.remoteMethod(
    'setPasswordDefault',
    {
      description: 'Set password of a user to default password "0000"',
      accepts: [
        {arg: 'id', type: 'string', required: true},
      ],
      http: {verb: 'get', path: '/:id/setPasswordDefault'},
    }
  );

  User.setPasswordDefault = function(userId, fn) {
    this.findById(userId, function(err, user) {
      if (err) {
        fn(err);
      } else {
        user.updateAttributes({ password: '0000' }, function(err1, result) {
          if (err1) {
            fn(err1);
          } else {
            fn(null, result);
  ...
  User.remoteMethod(
    'changePassword',
    {
      ...
      http: {verb: 'get', path: '/changePassword'},
    }
  );

  User.changePassword = function(oldPassword, newPassword, options, fn) {
    ...
    this.findById(userId, function(err, user) {
      if (err) {
        ...
        user.hasPassword(oldPassword, function(err1, isMatch) {
          if (isMatch) {
            user.updateAttributes({ password: newPassword }, function(err2, result2) {
              if (err2) {
                fn(err2);
              } else {
                fn(null, result2);
            ...
            const newErrMsg = 'User specified wrong current password !';
            const newErr = new Error(newErrMsg);
