...
master=''
if [ -n "$SERVICENAME" ] && [ -n "$MY_POD_NAME" ]; then
	PARALLEL=${PARALLEL:-2}
	id=`echo "$MY_POD_NAME" | sed -e 's/.*-\([0-9][0-9]*$\)/\1/'`
	statefulset=`echo "$MY_POD_NAME" | sed -e 's/\(.*\)-[0-9]*$/\1/'`
	domain="${SERVICENAME}.${MY_POD_NAMESPACE}.svc.cluster.local"
	master="$statefulset-$(($id / $PARALLEL * $PARALLEL))"

	export RABBITMQ_USE_LONGNAME=true
	export RABBITMQ_NODENAME="rabbit@${MY_POD_NAME}.${domain}"
fi
...
if [[ "$1" == rabbitmq* ]] && [ "$(id -u)" = '0' ]; then
	if [ "$1" = 'rabbitmq-server' ]; then
		...
		chown -R rabbitmq /config
	fi
	exec su-exec rabbitmq "$BASH_SOURCE" "$@"
fi

ssl_files=`ls /etc/ssl | grep -E '^(cacert|cert|key)\.pem' | wc -l`
if [ "${ssl_files}" -eq "3" ]; then
	RABBITMQ_SSL_CACERTFILE=/etc/ssl/cacert.pem
...
rabbit_env_config() {
	...
	for conf; do
		...
		case "$conf" in
			...
			auth_mechanisms)
				rawVal="['PLAIN', 'AMQPLAIN', 'EXTERNAL']"
				;;
...
if [ "$1" = 'rabbitmq-server' ] && [ "$shouldWriteConfig" ]; then
	...
	if [ -n "$MY_POD_NAME" ] && [ "$MY_POD_NAME" != "$master" ]; then
		rabbitConfig+=(
			"{ cluster_nodes, {['rabbit@${master}.${domain}'], disc} }"
		)
	fi
	...
	fullConfig+=( "{ rabbitmq_mqtt, $(rabbit_array "${mqttConfig[@]}") }" )
